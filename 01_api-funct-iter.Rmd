...
```{r ROMO data}
ROMO_data <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=ROMO&startMonth=1&startYear=2010&endMonth=12&endYear=2021")
unpacked_ROMO_data <- httr::content(ROMO_data, as = "text", encoding = "UTF-8") 
final_ROMO_data <- jsonlite::fromJSON(unpacked_ROMO_data)
```
...
```{r unit_visitation function}
unit_visitation <- function(unit_codes, start_month, start_year, end_month, end_year){
  raw_unit_data <- httr::GET(url = paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=", unit_codes,"&startMonth=", start_month, "&startYear=", start_year, "&endMonth=", end_month, "&endYear=", end_year))

# convert content to text
extracted_unit_data <- httr::content(raw_unit_data, as = "text", encoding = "UTF-8")

# parse text from JSON to data frame
final_unit_data <- jsonlite::fromJSON(extracted_unit_data)
...
```{r unit_visitation test for specific years}
unit_data_romo<- unit_visitation(unit_codes = "ROMO", 1, 1990, 12, 2021)
unit_data_ever<- unit_visitation(unit_codes = "EVER", 1, 1990, 12, 2021)
unit_data_thro<- unit_visitation(unit_codes = "THRO", 1, 1990, 12, 2021)
```
...
```{r update uni_visitation defaults}
unit_visitation <- function(unit_codes, start_month = 1, start_year, end_month = 12, end_year){
  raw_unit_data <- httr::GET(url = paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=", unit_codes,"&startMonth=", start_month, "&startYear=", start_year, "&endMonth=", end_month, "&endYear=", end_year))
...
unit_data_romo<- unit_visitation(unit_codes = "ROMO", start_year = 1990, end_year = 2021)
unit_data_ever<- unit_visitation(unit_codes = "EVER", start_year = 1990, end_year = 2021)
unit_data_thro<- unit_visitation(unit_codes = "THRO", start_year = 1990, end_year = 2021)
```
...
output_floop_parks <- vector("list", length = length(parks))

#for loop

for(i in 1:length(parks)){
  
  output_floop_parks[[i]] <-
    unit_visitation(unit_codes = parks[i], start_year = 1980, end_year = 2021)
  
}

#summarize results

summary(output_floop_parks)

#create single data set

multi_parks <- dplyr::bind_rows(output_floop_parks)
```
...
```{r map unit_visitation}
output_map_parks <- parks %>%
  map(~ unit_visitation(unit_codes = ., start_year = 1980, end_year = 2021))
