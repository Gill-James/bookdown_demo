...
plotly::ggplotly(
  ggplot(data=yearly) +
    ...
    ggtitle("Park Visitation vs Parkwide") +
    theme_bw(base_size=10)
)
```
...
# Mutate percent column and pivot to narrow
long_percent <- wide_data %>%
...
```{r exercise 4 data}
# Combine data to be annual only and create percent column
joined_data_annual <- joined_data %>%
  group_by(Year, UnitCode.x) %>%
  summarise(Park_Visitors = sum(RecreationVisitors.x), Parkwide_Visitors = sum(RecreationVisitors.y), Percent = ((sum(RecreationVisitors.x)/sum(RecreationVisitors.y)*100)))
```

```{r exercise 4 plot}
#Plot 
joined_data_annual_ggplot <- ggplot(data=joined_data_annual) +
    geom_point(aes(x = Year, y = Percent, color = UnitCode.x)) +
    geom_path(aes(x = Year, y = Percent, color = UnitCode.x)) +
    ylab("Percent of Total Annual Park Visits") +
    ggtitle("Percent of Total Annual Park Visits by Park") +
    theme_bw(base_size=10)
plotly::ggplotly(joined_data_annual_ggplot)
```
...
On average, Great Smoky Mountains National Park has the most visitaion. Looking at the mean percentage of total parkwide visitors from each individual park, Great Smoky has 3.36% of total park visitors which is the highest mean percent. This can also be seen in the plots from exercises 3 and 4.
```{r exercise 5}
aggregate(Percent~UnitCode.x, data = joined_data_annual[81:36,], mean)
```
